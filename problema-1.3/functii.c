...
void primaCol(int **a, int n)
{
	for(int i=0;i<n-1;++i)
		for(int j=i+1;j<n;++j)
		{
			if(a[i][0]>a[j][0]) interschimbareLinii(a,n,m,i,j);
		}
}
...
void primaLinie(int **a, int n)
{
	for(int j=0;j<n-1;++j)
		for(int k=j+1;k<n;++k)
		{
			if(a[0][j]>a[0][k]) interschimbareCol(a,n,m,j,k);
		}
}*/
...
void diagP(int **a, int n, int m)
{
	for(int i=0;i<n-1;++i)
		for(int j=i+1;j<n;++j)
		{
			if(a[i][i]>a[j][j]) interschimbareCol(a,n,m,i,j);
		}
}
	...
	for(int i=0;i<n-1;++i)
		for(int j=i+1;j<n;++j)
			if(a[i][m-i-1]>a[j][m-j-1])
				interschimbareCol(a,n,m,m-i-1,m-j-1);
}
...
void linieV(int **a, int n, int m)
{
	if(m%2==0)
	{
		printf("AM ZIS NUMAR IMPAR DE COLOANE!!!\n");
		return;
	}
	for(int i=0;i<n-1;++i)
		for(int j=i+1;j<n;++j)
			if(a[i][m/2]>a[j][m/2]) interschimbareLinii(a,n,m,i,j);
}
